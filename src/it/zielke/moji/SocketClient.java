...
public class SocketClient {
     ...
     *
     * @param server host name or IP of the MOSS server
     * @param port port of the MOSS server
     */
     ...
     * @param server host name or IP of the MOSS server
     * @param port port of the MOSS server
     * @param language programming language of all source files
     */
     ...
     * @throws IOException if setting up the input streams fails
     * @throws SecurityException if a connection is disallowed by your security
     * manager
     */
     ...
     * @throws IOException if it was not possible to read from the socket
     */
     ...
     *
     * @throws MossException if the MOSS server returns an error
     * @throws IOException if there is a (network) error while reading the
     * server response
     * @throws UnknownHostException if host resolution fails
     */
     ...
     *
     * @param objects object array consisting of parameters which should be sent
     * to the server
     * @return the generated string which was actually sent to the server
     * @throws MossException if the server response is unexpected
     */
     ...
     * Generic function for sending all kinds of commands to the server.
     *
     * @param strings string array consisting of parameters which should be sent
     * to the server
     * @return the generated string which was actually sent to the server
     ...
     * @throws MossException if this language is not supported by the MOSS
     * server
     * @throws IOException if there is an error while reading the server answer
     */
     ...
     * source files.
     *
     * @param language the programming language of all source files
     * @throws MossException if this language is not supported by the MOSS
     * server
     * @throws IOException if there is an error while reading the server answer
     */
     ...
     *
     * @throws MossException if there is an error when receiving results from
     * the MOSS server
    ...
    public void sendQuery() throws MossException, IOException {
        ...
        if (null != result && result.toLowerCase(Locale.ENGLISH).startsWith("http")) {
            try {
     ...
     *
     * @param language the programming language in which source file is written
     * in. Valid programming languages are: c, cc, java, ml, pascal, ada, lisp,
     * schema, haskell, fortran, ascii, vhdl, perl, matlab, python, mips,
     * prolog, spice, vb, csharp, modula2, a8086, javascript and plsql.
     */
     ...
     * the original client.
     *
     * @param optM times a passage may appear before it is ignored
     */
     ...
     * 250.
     *
     * @param optN how many matches to display
     */
     ...
     *
     * @param optX 0 or 1. 0: regular MOSS server, 1: experimental MOSS server.
     */
     ...
     *
     * @param userID your personalized ID to authenticate with. It should look
     * like /[0-9]+/ and is available on the MOSS page located at
     ...
     *
     * @param file the source code file to be uploaded
     ...
     *
     * @param file the base file to be uploaded
     * @throws IOException if the file could not be read
     */
     ...
     *
     * @param file the source code file to be uploaded
     * @param isBaseFile true is base file. false otherwise.
     * @throws IOException if the file could not be read
     */
    public void uploadFile(File file, boolean isBaseFile) throws IOException {
        if (currentStage != Stage.AWAITING_FILES && currentStage != Stage.AWAITING_QUERY) {
            throw new RuntimeException(
